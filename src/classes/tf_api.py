    ...
    def to_code_tensor(self, var_name, low_precision=False):
        ...
        if dtype.is_floating:
            ...
            code += "%s = tf.saturate_cast(" \
                ...
                "dtype=tf.%s)\n" % (var_tensor_name, shape,
                                    self.minv, self.maxv + 1, dtype.name)
...
class TFAPI(API):
    def __init__(self, record) -> None:
        ...
        self.args = TFAPI.generate_args_from_record(record)
        try:
    ...
    def my_to_code(self, prefix="arg", res_name="", low_precision=False, **kwargs) -> str:
