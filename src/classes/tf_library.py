    ...
    def test_with_oracle(self, api: TFAPI, oracle: OracleType):
        ...
        if oracle == OracleType.CRASH:
            ...
            if bug_flag:
                ...
                with open('benchmarkingDLFuzzers/dataset/tf-v2.3.0.csv', 'a', newline='\n') as fd:
                    writer_object = csv.writer(fd)
                    writer_object.writerow(data_)
            if success_flag:
                data_ = [api.api, arg_code, arg_str,
                         api_code, new_name, 'success']
                with open('benchmarkingDLFuzzers/dataset/tf-v2.3.0.csv', 'a', newline='\n') as fd:
                    writer_object = csv.writer(fd)
                    writer_object.writerow(data_)
            ...
            if success_flag:
                ...
                with open('benchmarkingDLFuzzers/dataset/tf-v2.3.0.csv', 'a', newline='\n') as fd:
                    ...
                    writer_object.writerow(data_)
            # if success_flag:
            #     data_ = [api.api, arg_code, 'success']
