...
try:
  try:
    with tf.device('/CPU'):
      map_func_0_tensor = tf.complex(tf.random.uniform([10, 5], dtype=tf.float64),tf.random.uniform([10, 5], dtype=tf.float64))
      map_func_0 = tf.identity(map_func_0_tensor)
      map_func_1_tensor = tf.saturate_cast(tf.random.uniform([10, 5, 0], minval=0, maxval=257, dtype=tf.int64), dtype=tf.uint16)
  ...
  try:
    with tf.device('/GPU:0'):
      map_func_0 = tf.identity(map_func_0_tensor)
      map_func_0 = tf.cast(map_func_0, tf.complex128)
      map_func_1 = tf.identity(map_func_1_tensor)
      map_func_1 = tf.cast(map_func_1, tf.uint16)
      map_func = [map_func_0,map_func_1,]
