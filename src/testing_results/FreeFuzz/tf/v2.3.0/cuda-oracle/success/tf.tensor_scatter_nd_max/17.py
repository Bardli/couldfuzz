...
try:
  try:
    with tf.device('/CPU'):
      tensor_tensor = tf.random.uniform([1, 35, 512, 12], dtype=tf.float32)
      ...
      updates_tensor = tf.complex(tf.random.uniform([0, 16, 11, 1], dtype=tf.float64),tf.random.uniform([0, 16, 11, 1], dtype=tf.float64))
  ...
  try:
    with tf.device('/GPU:0'):
      ...
      updates = tf.identity(updates_tensor)
      updates = tf.cast(updates, tf.complex128)
