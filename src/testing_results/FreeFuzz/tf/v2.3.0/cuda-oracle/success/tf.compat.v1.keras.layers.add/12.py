...
try:
  try:
    with tf.device('/CPU'):
      ...
      inputs_1_tensor = tf.saturate_cast(tf.random.uniform([2, 2], minval=-256, maxval=257, dtype=tf.int64), dtype=tf.int16)
      inputs_1 = tf.identity(inputs_1_tensor)
      inputs_2_tensor = tf.random.uniform([2, 2], dtype=tf.float16)
  ...
  try:
    with tf.device('/GPU:0'):
      ...
      inputs_1 = tf.identity(inputs_1_tensor)
      inputs_1 = tf.cast(inputs_1, tf.int16)
      inputs_2 = tf.identity(inputs_2_tensor)
      inputs_2 = tf.cast(inputs_2, tf.float16)
      inputs = [inputs_0,inputs_1,inputs_2,]
