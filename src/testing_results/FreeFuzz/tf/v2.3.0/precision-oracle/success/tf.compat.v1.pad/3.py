...
try:
  try:
    tensor_tensor = tf.random.uniform([12, 2, 256, 513], dtype=tf.float16)
    tensor = tf.identity(tensor_tensor)
    paddings_tensor = tf.saturate_cast(tf.random.uniform([4, 2], minval=-256, maxval=257, dtype=tf.int64), dtype=tf.int8)
