...
try:
  try:
    map_func_0_tensor = tf.saturate_cast(tf.random.uniform([10, 5], minval=-256, maxval=257, dtype=tf.int64), dtype=tf.int8)
    map_func_0 = tf.identity(map_func_0_tensor)
    map_func_1_tensor = tf.complex(tf.random.uniform([10, 5], dtype=tf.float32),tf.random.uniform([10, 5], dtype=tf.float32))
  ...
  try:
    map_func_0 = tf.identity(map_func_0_tensor)
    map_func_0 = tf.cast(map_func_0, tf.int32)
    map_func_1 = tf.identity(map_func_1_tensor)
