...
try:
  try:
    map_func_0_tensor = tf.saturate_cast(tf.random.uniform([10, 5], minval=0, maxval=256, dtype=tf.int64), dtype=tf.uint8)
    map_func_0 = tf.identity(map_func_0_tensor)
    map_func_1_tensor = tf.saturate_cast(tf.random.uniform([10], minval=0, maxval=256, dtype=tf.int64), dtype=tf.uint8)
  ...
  try:
    map_func_0 = tf.identity(map_func_0_tensor)
    map_func_0 = tf.cast(map_func_0, tf.uint8)
    map_func_1 = tf.identity(map_func_1_tensor)
    map_func_1 = tf.cast(map_func_1, tf.uint8)
    map_func = [map_func_0,map_func_1,]
