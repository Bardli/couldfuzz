...
try:
  try:
    params_tensor = tf.random.uniform([1, 20, 512], dtype=tf.float16)
    params = tf.identity(params_tensor)
    indices_tensor = tf.saturate_cast(tf.random.uniform([4], minval=-256, maxval=257, dtype=tf.int64), dtype=tf.int8)
