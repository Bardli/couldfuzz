...
try:
  try:
    tensor_tensor = tf.saturate_cast(tf.random.uniform([1, 512, 768], minval=0, maxval=256, dtype=tf.int64), dtype=tf.uint8)
    tensor = tf.identity(tensor_tensor)
    indices_tensor = tf.saturate_cast(tf.random.uniform([8, 2], minval=-256, maxval=257, dtype=tf.int64), dtype=tf.int8)
    indices = tf.identity(indices_tensor)
    updates_tensor = tf.random.uniform([8, 768], dtype=tf.float16)
