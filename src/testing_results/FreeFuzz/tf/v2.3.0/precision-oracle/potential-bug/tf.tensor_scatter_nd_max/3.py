...
try:
  try:
    tensor_tensor = tf.random.uniform([1, 512, 768], dtype=tf.float16)
    tensor = tf.identity(tensor_tensor)
    indices_tensor = tf.saturate_cast(tf.random.uniform([35, 2], minval=-256, maxval=257, dtype=tf.int64), dtype=tf.int8)
    indices = tf.identity(indices_tensor)
    updates_tensor = tf.random.uniform([35, 768], dtype=tf.float16)
