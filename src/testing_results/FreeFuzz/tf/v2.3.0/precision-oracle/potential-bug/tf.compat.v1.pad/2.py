...
try:
  try:
    tensor_tensor = tf.saturate_cast(tf.random.uniform([1, 29], minval=-256, maxval=257, dtype=tf.int64), dtype=tf.int8)
    tensor = tf.identity(tensor_tensor)
    paddings_tensor = tf.saturate_cast(tf.random.uniform([2, 2], minval=-256, maxval=257, dtype=tf.int64), dtype=tf.int8)
